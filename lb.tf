...
resource "yandex_alb_backend_group" "web_bg" {
  ...
  http_backend {
    ...
    # --- ИСПРАВЛЕНИЕ: Более мягкие настройки healthcheck ---
    healthcheck {
      timeout  = "5s"      # Увеличили таймаут
      interval = "10s"     # Увеличили интервал
      healthy_threshold   = 2 # Сколько успешных проверок нужно, чтобы считать живым
      unhealthy_threshold = 2 # Сколько неудачных, чтобы считать мертвым
      http_healthcheck {
        path = "/"
      }
    }
...
resource "yandex_alb_load_balancer" "web_balancer" {
  ...
  # ИСПРАВЛЕНИЕ: Явное ожидание создания ВМ
  # Terraform не будет создавать ALB, пока не будут готовы web_1 и web_2
  depends_on = [
